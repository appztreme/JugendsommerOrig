...
var txtReceipt = "Hallo liebe Eltern,\r\n\r\nanbei findet ihr den Überweisungsschein für das Sommerprogramm 2021.\r\nWichtig: Bei der Überweisung bitte die Überweisungsnummer und den Namen des Kindes angeben (Einzahlungsfrist bei Sicht).\r\nFür unsere Buchhaltung bitten wir euch, jeden ÜberweisungsscheinscBei der Überweisung bitte die Überweisungsnummer und den Namen des Kindes angeben.\r\nÜberweisungsschein\r\nKontodaten:Raiffeisenkassa Bozen\r\nIBAN: IT 09X 08081 11610 000306005853";
var htmlReceiptStart = "<html><body><p>Hallo liebe Eltern,<br/ >anbei findet ihr den Überweisungsschein für das Sommerprogramm 2021.<br /><strong>Wichtig:</strong> Bei der Überweisung bitte die <span style='color:#ffa500'><strong>Überweisungsnummer</strong></span> und den <span style='color:#ffa500'><strong>Namen des Kindes</strong></span> angeben (Einzahlungsfrist bei Sicht).<br /><br /></strong><br /><span style='color:#ffa500'><strong>Überweisungsschein:</strong></span><br />Kontodaten:<br /><strong>Raiffeisenkassa Bozen<br />IBAN: IT 09X 08081 11610 000306005853</strong><br />";
...
const getConfirmationPDF_JDUL = async function(instance, reservations, config, lang) {
	...
	for(let i = 0; i < childrenUnique.length; i++) {
		...
		if(lang === 'de') {
			...
			doc.moveDown(1);
			doc.font('Helvetica').text("con la presente il " + config.member + " conferma l’iscrizione di " + child + ", per le settimane:", { align: 'left', width: 430 });
			doc.moveDown(1);
			fee = 0;
			index = 0;
			//doc.font('Helvetica-Bold').text(registrationsPerChild[0].activityId.eventId.name + ' ' + registrationsPerChild[0].activityId.eventId.location, { align: 'left', width: 430 });
			...
			for(let reg of registrationsPerChild) {
				if(index > 0) doc.font('Helvetica').text(", ", {continued: true});
				doc.font('Helvetica').text(reg.activityId.name_it + ' (' + moment(reg.activityId.startDate).format('DD.MM') + '-' + moment(reg.activityId.endDate).format('DD.MM.YYYY') + ')', { continued: true });
				//doc.moveDown(1);
				fee += calculateReceiptFee(reg, reg.activityId);
				index += 1;
			}
...
const getReservationAttachment = function(firstNameChild, activities, reservation){
	...
	doc.moveDown(0.5);
	doc.text("Du erhälst innerhalb Mai noch eine weitere E-Mail mit detaillierteren Informationen.");
	doc.moveDown(0.5);
...
function getActivityTable(activities) {
	...
	for(var i=0; i<activities.length; i++) {
		...
		tblStart += '<tr><td style="border: 1px solid gray; padding: 2px;">' + activities[i].eventId.location + ' - ' + activities[i].eventId.name + '(' + moment(activities[i].startDate).format("DD.MM") + ' - ' + moment(activities[i].endDate).format("DD.MM") + ')' + '<br />' + activities[i].eventId.location_it + ' - ' + activities[i].eventId.name_it + '(' + moment(activities[i].startDate).format("DD.MM") + ' - ' + moment(activities[i].endDate).format("DD.MM") + ')' + '</td><td style="border: 1px solid gray; padding: 2px">' + activities[i].name + '<br />' + activities[i].name_it + '</td><td style="border: 1px solid gray; padding: 2px;">' + calculateFee(activities[i]) + '</td></tr>';
	}
